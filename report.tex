...
Automatic modulation classification (AMC) is a key function in intelligent receivers and spectrum monitoring systems. This project presents a baseline convolutional neural network (CNN) for classifying modulation types directly from raw I/Q samples using the RadioML 2016 dataset. The dataset provides labeled signals across multiple modulation schemes and signal-to-noise ratio (SNR) conditions, enabling evaluation of both low- and high-SNR regimes. The proposed approach performs per-sample power normalization, encodes modulation labels, and trains a 1D CNN on I/Q sequences of length 128 with a stratified train/validation/test split. The model is optimized with AdamW and assessed using accuracy curves, confusion matrices, and SNR-wise performance analysis. The overall test accuracy achieved is approximately 0.574, with performance improving at higher SNRs and common confusions among closely related modulation families. The results confirm that CNNs are effective for AMC on raw I/Q data while highlighting the sensitivity to SNR distribution and modulation similarity, motivating future improvements in architecture and data augmentation.
...
Figure~\ref{fig:amc-pipeline} summarizes the end-to-end pipeline from raw samples to predicted modulation labels.
...
This project implemented a baseline CNN for automatic modulation classification on the RadioML 2016 dataset. The model achieved an overall test accuracy of about 0.574 and demonstrated clear SNR-dependent performance trends.

\textbf{Contributions:}
\begin{itemize}
    \item Implemented a reproducible preprocessing and stratified data-splitting pipeline for RadioML 2016.
